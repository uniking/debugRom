...
class ArgArray {
 public:
  ...
  void VarArgsShowArg(const art::ScopedObjectAccessAlreadyRunnable& soa,
	...
	void JValuesShowArg(const art::ScopedObjectAccessAlreadyRunnable& soa,
...
void ShowVarArgs(const art::ScopedObjectAccessAlreadyRunnable& soa,
                                           const char* jniMethod,
                                           jmethodID mid,
                                           va_list args){
	 //if(ArtMethod::IsJNIMethodPrint()){
	 			ArtMethod* method = art::jni::DecodeArtMethod(mid);
   			uint32_t shorty_len = 0;
         const char* shorty =
               method->GetInterfaceMethodIfProxy(kRuntimePointerSize)->GetShorty(&shorty_len);
         JValue result;
...
void ShowJValue(const art::ScopedObjectAccessAlreadyRunnable& soa,
                                           const char* jniMethod,
                                           jmethodID mid,
                                           const jvalue* args){
    //if(ArtMethod::IsJNIMethodPrint()){
...
void ShowJniField(const char* jniMethodName,jfieldID field) {
		//if(ArtMethod::IsJNIMethodPrint()){
			ArtField* f = art::jni::DecodeArtField(field);
